...
int main(int argc, char *argv[]) {
  ...
  //Abertura do arquivo que contém a lista de cada conjunto de instâncias (cada conjunto possui sua própria lista de instâncias)
  ...
  for (unsigned i = 0; i < listSetInstances.size(); i++) {
    ...
    //Abertura do arquivo que armazena os dados da solução de cada instância de um determinado conjunto
...
//Execução dos Testes para as instâncias do conjunto i atual   //listFileInstances.size()
    for (unsigned j = 10; j < listFileInstances.size(); j++) {
      ...
      fileNameSolutions = "Solucoes/" + listSetInstances[i] + "/Solucao_" + listFileInstances[j].c_str();
      ...
      if (dataFileSolutions.is_open()) {
        ...
        cout << "Não foi possivel abrir o arquivo dataFileSolutions!: " << fileNameSolutions << endl;
        dataFileSolutions.clear();
        exit(1);
      }

      fileNameSolutions.clear();
      fileNameSolutions = "Melhores_Solucoes/" + listSetInstances[i] + "/Dados_Tempo_" + listFileInstances[j].c_str();
      ...
      if (dataFileBestSolutions.is_open()) {
        ...
        cout << "Não foi possivel abrir o arquivo dataFileBestSolutions!: " << fileNameSolutions << endl;
      ...
      do {
        ...
        if (algorithm.getBestFitness() < FO_Star) {
          ...
          dataFileBestSolutions << " " << setprecision(12) << (algorithm.getBestFitness() / 10.0) * -1 << "   "
                                << setprecision(12) << TempoFO_Star << "  " << algorithm.getBestFitness() << "  "
                                << quantIteracoes << "  " << "\n";
      ...
      dataFileWrite << " " << listFileInstances[j].c_str() << "   " << setprecision(12) << (FO_Star / 10.0) * -1
                    << "   " << setprecision(12) << TempoExecTotal << "  " << algorithm.getBestFitness() << "  "
                    << quantIteracoes << "  " << bestIteration << "  " << TempoFO_Star << "  " << seed << "\n";
